from PySide2 import QtCore, QtGui, QtWidgets

from PySide2.QtCore import (
    Qt,
)
from PySide2.QtGui import (
    QIcon,
)
from PySide2.QtWidgets import (
    QHBoxLayout,
    QSizePolicy,
    QSpacerItem,
    QWidget
)

class Ui_Script:
    def __init__(self, Script:QWidget):

        # Set window parameters
        Script.setWindowTitle("ASL Tools - Script")
        Script.setWindowIcon(QIcon(":/images/icon.png"))
        Script.setWindowFlag(Qt.Window)
        Script.resize(800, 540)
        Script.move(0, 0)

        # Main frame
        self.frame_h_layout = QWidget(Script)
        self.frame_h_layout.setObjectName("scriptwindow")
        # Spacer
        spacer = QSpacerItem(40, 20, QSizePolicy.Expanding, QSizePolicy.Minimum)
        # Set widgets in a horizontal widget
        self.horizontalLayout_2 = QHBoxLayout(self.frame_h_layout)
        self.horizontalLayout_2.addItem(spacer)
        self.gridLayout = QtWidgets.QGridLayout()
        self.gridLayout.setSizeConstraint(QtWidgets.QLayout.SetNoConstraint)
        self.gridLayout.setObjectName("gridLayout")
        self.list = QtWidgets.QListWidget(self.frame_h_layout)
        self.list.setEditTriggers(QtWidgets.QAbstractItemView.DoubleClicked)
        self.list.setObjectName("list")
        self.gridLayout.addWidget(self.list, 3, 0, 1, 1)
        self.gridLayout_4 = QtWidgets.QGridLayout()
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.label_i_pmus_inc = QtWidgets.QLabel(self.frame_h_layout)
        self.label_i_pmus_inc.setObjectName("label_i_pmus_inc")
        self.gridLayout_4.addWidget(self.label_i_pmus_inc, 6, 0, 1, 1)
        self.label_i_pmus = QtWidgets.QLabel(self.frame_h_layout)
        self.label_i_pmus.setObjectName("label_i_pmus")
        self.gridLayout_4.addWidget(self.label_i_pmus, 5, 0, 1, 1)
        spacerItem1 = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.gridLayout_4.addItem(spacerItem1, 4, 0, 1, 1)
        self.label_i_pmus_hld = QtWidgets.QLabel(self.frame_h_layout)
        self.label_i_pmus_hld.setObjectName("label_i_pmus_hld")
        self.gridLayout_4.addWidget(self.label_i_pmus_hld, 7, 0, 1, 1)
        spacerItem2 = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.gridLayout_4.addItem(spacerItem2, 16, 0, 1, 1)
        spacerItem3 = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.gridLayout_4.addItem(spacerItem3, 14, 0, 1, 1)
        self.i_pmus_inc = QtWidgets.QSpinBox(self.frame_h_layout)
        self.i_pmus_inc.setMaximum(100)
        self.i_pmus_inc.setObjectName("i_pmus_inc")
        self.gridLayout_4.addWidget(self.i_pmus_inc, 6, 1, 1, 1)
        self.i_pmus_rel = QtWidgets.QSpinBox(self.frame_h_layout)
        self.i_pmus_rel.setMaximum(100)
        self.i_pmus_rel.setObjectName("i_pmus_rel")
        self.gridLayout_4.addWidget(self.i_pmus_rel, 8, 1, 1, 1)
        spacerItem4 = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.gridLayout_4.addItem(spacerItem4, 9, 0, 1, 1)
        self.i_pmus = QtWidgets.QSpinBox(self.frame_h_layout)
        self.i_pmus.setMaximum(100)
        self.i_pmus.setObjectName("i_pmus")
        self.gridLayout_4.addWidget(self.i_pmus, 5, 1, 1, 1)
        self.i_pmus_hld = QtWidgets.QSpinBox(self.frame_h_layout)
        self.i_pmus_hld.setMaximum(100)
        self.i_pmus_hld.setObjectName("i_pmus_hld")
        self.gridLayout_4.addWidget(self.i_pmus_hld, 7, 1, 1, 1)
        self.compliance = QtWidgets.QSpinBox(self.frame_h_layout)
        self.compliance.setMaximum(500)
        self.compliance.setObjectName("compliance")
        self.gridLayout_4.addWidget(self.compliance, 0, 1, 1, 1)
        self.label_repetitions = QtWidgets.QLabel(self.frame_h_layout)
        self.label_repetitions.setObjectName("label_repetitions")
        self.gridLayout_4.addWidget(self.label_repetitions, 17, 0, 1, 1)
        self.repetitions = QtWidgets.QSpinBox(self.frame_h_layout)
        self.repetitions.setMaximum(9999)
        self.repetitions.setObjectName("repetitions")
        self.gridLayout_4.addWidget(self.repetitions, 17, 1, 1, 1)
        self.respi_rate = QtWidgets.QSpinBox(self.frame_h_layout)
        self.respi_rate.setMaximum(500)
        self.respi_rate.setObjectName("respi_rate")
        self.gridLayout_4.addWidget(self.respi_rate, 2, 1, 1, 1)
        self.resistance = QtWidgets.QSpinBox(self.frame_h_layout)
        self.resistance.setMaximum(500)
        self.resistance.setObjectName("resistance")
        self.gridLayout_4.addWidget(self.resistance, 1, 1, 1, 1)
        self.label_e_pmus = QtWidgets.QLabel(self.frame_h_layout)
        self.label_e_pmus.setObjectName("label_e_pmus")
        self.gridLayout_4.addWidget(self.label_e_pmus, 10, 0, 1, 1)
        self.e_pmus = QtWidgets.QSpinBox(self.frame_h_layout)
        self.e_pmus.setMaximum(100)
        self.e_pmus.setObjectName("e_pmus")
        self.gridLayout_4.addWidget(self.e_pmus, 10, 1, 1, 1)
        self.label_e_pmus_inc = QtWidgets.QLabel(self.frame_h_layout)
        self.label_e_pmus_inc.setObjectName("label_e_pmus_inc")
        self.gridLayout_4.addWidget(self.label_e_pmus_inc, 11, 0, 1, 1)
        self.e_pmus_hld = QtWidgets.QSpinBox(self.frame_h_layout)
        self.e_pmus_hld.setMaximum(100)
        self.e_pmus_hld.setObjectName("e_pmus_hld")
        self.gridLayout_4.addWidget(self.e_pmus_hld, 12, 1, 1, 1)
        self.e_pmus_rel = QtWidgets.QSpinBox(self.frame_h_layout)
        self.e_pmus_rel.setMaximum(100)
        self.e_pmus_rel.setObjectName("e_pmus_rel")
        self.gridLayout_4.addWidget(self.e_pmus_rel, 13, 1, 1, 1)
        self.label_e_pmus_hld = QtWidgets.QLabel(self.frame_h_layout)
        self.label_e_pmus_hld.setObjectName("label_e_pmus_hld")
        self.gridLayout_4.addWidget(self.label_e_pmus_hld, 12, 0, 1, 1)
        self.e_pmus_inc = QtWidgets.QSpinBox(self.frame_h_layout)
        self.e_pmus_inc.setMaximum(100)
        self.e_pmus_inc.setObjectName("e_pmus_inc")
        self.gridLayout_4.addWidget(self.e_pmus_inc, 11, 1, 1, 1)
        self.label_e_pmus_rel = QtWidgets.QLabel(self.frame_h_layout)
        self.label_e_pmus_rel.setObjectName("label_e_pmus_rel")
        self.gridLayout_4.addWidget(self.label_e_pmus_rel, 13, 0, 1, 1)
        self.label_resistance = QtWidgets.QLabel(self.frame_h_layout)
        self.label_resistance.setObjectName("label_resistance")
        self.gridLayout_4.addWidget(self.label_resistance, 1, 0, 1, 1)
        self.label_i_pmus_rel = QtWidgets.QLabel(self.frame_h_layout)
        self.label_i_pmus_rel.setObjectName("label_i_pmus_rel")
        self.gridLayout_4.addWidget(self.label_i_pmus_rel, 8, 0, 1, 1)
        self.label_crf = QtWidgets.QLabel(self.frame_h_layout)
        self.label_crf.setObjectName("label_crf")
        self.gridLayout_4.addWidget(self.label_crf, 15, 0, 1, 1)
        self.label_respi_rate = QtWidgets.QLabel(self.frame_h_layout)
        self.label_respi_rate.setObjectName("label_respi_rate")
        self.gridLayout_4.addWidget(self.label_respi_rate, 2, 0, 1, 1)
        self.crf = QtWidgets.QDoubleSpinBox(self.frame_h_layout)
        self.crf.setDecimals(1)
        self.crf.setSingleStep(0.1)
        self.crf.setObjectName("crf")
        self.gridLayout_4.addWidget(self.crf, 15, 1, 1, 1)
        self.label_compliance = QtWidgets.QLabel(self.frame_h_layout)
        self.label_compliance.setObjectName("label_compliance")
        self.gridLayout_4.addWidget(self.label_compliance, 0, 0, 1, 1)
        self.label_repetitions_total = QtWidgets.QLabel(self.frame_h_layout)
        self.label_repetitions_total.setEnabled(True)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.label_repetitions_total.sizePolicy().hasHeightForWidth())
        self.label_repetitions_total.setSizePolicy(sizePolicy)
        self.label_repetitions_total.setMinimumSize(QtCore.QSize(40, 0))
        self.label_repetitions_total.setObjectName("label_repetitions_total")
        self.gridLayout_4.addWidget(self.label_repetitions_total, 17, 2, 1, 1)
        self.gridLayout.addLayout(self.gridLayout_4, 3, 1, 1, 1)
        self.horizontalLayout = QtWidgets.QHBoxLayout()
        self.horizontalLayout.setObjectName("horizontalLayout")
        self.btn_add = QtWidgets.QToolButton(self.frame_h_layout)
        self.btn_add.setEnabled(True)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/images/add.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_add.setIcon(icon)
        self.btn_add.setIconSize(QtCore.QSize(32, 32))
        self.btn_add.setAutoRaise(True)
        self.btn_add.setObjectName("btn_add")
        self.horizontalLayout.addWidget(self.btn_add)
        self.btn_remove = QtWidgets.QToolButton(self.frame_h_layout)
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap(":/images/remove.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_remove.setIcon(icon1)
        self.btn_remove.setIconSize(QtCore.QSize(32, 32))
        self.btn_remove.setAutoRaise(True)
        self.btn_remove.setObjectName("btn_remove")
        self.horizontalLayout.addWidget(self.btn_remove)
        self.btn_move_up = QtWidgets.QToolButton(self.frame_h_layout)
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap(":/images/arrow-up.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_move_up.setIcon(icon2)
        self.btn_move_up.setIconSize(QtCore.QSize(32, 32))
        self.btn_move_up.setAutoRaise(True)
        self.btn_move_up.setObjectName("btn_move_up")
        self.horizontalLayout.addWidget(self.btn_move_up)
        self.btn_move_down = QtWidgets.QToolButton(self.frame_h_layout)
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap(":/images/arrow-down.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_move_down.setIcon(icon3)
        self.btn_move_down.setIconSize(QtCore.QSize(32, 32))
        self.btn_move_down.setAutoRaise(True)
        self.btn_move_down.setObjectName("btn_move_down")
        self.horizontalLayout.addWidget(self.btn_move_down)
        self.gridLayout.addLayout(self.horizontalLayout, 1, 0, 1, 1)
        self.label_file = QtWidgets.QLabel(self.frame_h_layout)
        self.label_file.setTextFormat(QtCore.Qt.PlainText)
        self.label_file.setAlignment(QtCore.Qt.AlignCenter)
        self.label_file.setObjectName("label_file")
        self.gridLayout.addWidget(self.label_file, 0, 0, 1, 2)
        self.horizontalLayout_3 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_3.setObjectName("horizontalLayout_3")
        self.btn_open = QtWidgets.QToolButton(self.frame_h_layout)
        icon4 = QtGui.QIcon()
        icon4.addPixmap(QtGui.QPixmap(":/images/open_file.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_open.setIcon(icon4)
        self.btn_open.setIconSize(QtCore.QSize(32, 32))
        self.btn_open.setAutoRaise(True)
        self.btn_open.setObjectName("btn_open")
        self.horizontalLayout_3.addWidget(self.btn_open)
        self.btn_save = QtWidgets.QToolButton(self.frame_h_layout)
        icon5 = QtGui.QIcon()
        icon5.addPixmap(QtGui.QPixmap(":/images/save.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_save.setIcon(icon5)
        self.btn_save.setIconSize(QtCore.QSize(32, 32))
        self.btn_save.setAutoRaise(True)
        self.btn_save.setArrowType(QtCore.Qt.NoArrow)
        self.btn_save.setObjectName("btn_save")
        self.horizontalLayout_3.addWidget(self.btn_save)
        self.btn_export = QtWidgets.QToolButton(self.frame_h_layout)
        self.btn_export.setCursor(QtCore.Qt.ArrowCursor)
        icon6 = QtGui.QIcon()
        icon6.addPixmap(QtGui.QPixmap(":/images/export.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.btn_export.setIcon(icon6)
        self.btn_export.setIconSize(QtCore.QSize(32, 32))
        self.btn_export.setCheckable(False)
        self.btn_export.setAutoRaise(True)
        self.btn_export.setObjectName("btn_export")
        self.horizontalLayout_3.addWidget(self.btn_export)
        self.gridLayout.addLayout(self.horizontalLayout_3, 1, 1, 1, 1)
        self.line_3 = QtWidgets.QFrame(self.frame_h_layout)
        self.line_3.setFrameShape(QtWidgets.QFrame.HLine)
        self.line_3.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line_3.setObjectName("line_3")
        self.gridLayout.addWidget(self.line_3, 2, 0, 1, 2)
        self.horizontalLayout_2.addLayout(self.gridLayout)
        spacerItem5 = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        self.horizontalLayout_2.addItem(spacerItem5)

        self.retranslateUi(Script)
        QtCore.QMetaObject.connectSlotsByName(Script)
        Script.setTabOrder(self.btn_add, self.btn_remove)
        Script.setTabOrder(self.btn_remove, self.btn_move_up)
        Script.setTabOrder(self.btn_move_up, self.btn_move_down)
        Script.setTabOrder(self.btn_move_down, self.list)
        Script.setTabOrder(self.list, self.btn_open)
        Script.setTabOrder(self.btn_open, self.btn_save)
        Script.setTabOrder(self.btn_save, self.btn_export)
        Script.setTabOrder(self.btn_export, self.compliance)
        Script.setTabOrder(self.compliance, self.resistance)
        Script.setTabOrder(self.resistance, self.respi_rate)
        Script.setTabOrder(self.respi_rate, self.i_pmus)
        Script.setTabOrder(self.i_pmus, self.i_pmus_inc)
        Script.setTabOrder(self.i_pmus_inc, self.i_pmus_hld)
        Script.setTabOrder(self.i_pmus_hld, self.i_pmus_rel)
        Script.setTabOrder(self.i_pmus_rel, self.e_pmus)
        Script.setTabOrder(self.e_pmus, self.e_pmus_inc)
        Script.setTabOrder(self.e_pmus_inc, self.e_pmus_hld)
        Script.setTabOrder(self.e_pmus_hld, self.e_pmus_rel)
        Script.setTabOrder(self.e_pmus_rel, self.crf)
        Script.setTabOrder(self.crf, self.repetitions)

    def retranslateUi(self, Script):
        Script.setWindowTitle(QtWidgets.QApplication.translate("Script", "Form", None, -1))
        self.label_i_pmus_inc.setText(QtWidgets.QApplication.translate("Script", "Pmus inspi aug (%)", None, -1))
        self.label_i_pmus.setText(QtWidgets.QApplication.translate("Script", "Pmus inspi (cmH2O)", None, -1))
        self.label_i_pmus_hld.setText(QtWidgets.QApplication.translate("Script", "Pmus inspi plateau (%)", None, -1))
        self.label_repetitions.setText(QtWidgets.QApplication.translate("Script", "Répétitions", None, -1))
        self.label_e_pmus.setText(QtWidgets.QApplication.translate("Script", "Pmus expi (cmH2O)", None, -1))
        self.label_e_pmus_inc.setText(QtWidgets.QApplication.translate("Script", "Pmus expi aug (%)", None, -1))
        self.label_e_pmus_hld.setText(QtWidgets.QApplication.translate("Script", "Pmus expi plateau (%)", None, -1))
        self.label_e_pmus_rel.setText(QtWidgets.QApplication.translate("Script", "Pmus expi dim (%)", None, -1))
        self.label_resistance.setText(QtWidgets.QApplication.translate("Script", "Résistances", None, -1))
        self.label_i_pmus_rel.setText(QtWidgets.QApplication.translate("Script", "Pmus inspi dim (%)", None, -1))
        self.label_crf.setText(QtWidgets.QApplication.translate("Script", "CRF (L)", None, -1))
        self.label_respi_rate.setText(QtWidgets.QApplication.translate("Script", "Fréquence respi", None, -1))
        self.label_compliance.setText(QtWidgets.QApplication.translate("Script", "Compliance", None, -1))
        self.label_repetitions_total.setText(QtWidgets.QApplication.translate("Script", "/20", None, -1))
        self.btn_add.setToolTip(QtWidgets.QApplication.translate("Script", "Ajouter une simulation", None, -1))
        self.btn_add.setText(QtWidgets.QApplication.translate("Script", "Ajouter", None, -1))
        self.btn_remove.setToolTip(QtWidgets.QApplication.translate("Script", "Supprimer une simulation", None, -1))
        self.btn_remove.setText(QtWidgets.QApplication.translate("Script", "Supprimer", None, -1))
        self.btn_move_up.setToolTip(QtWidgets.QApplication.translate("Script", "Déplacer une simulation vers le haut", None, -1))
        self.btn_move_up.setText(QtWidgets.QApplication.translate("Script", "Monter", None, -1))
        self.btn_move_down.setToolTip(QtWidgets.QApplication.translate("Script", "Déplacer ue simulation vers le bas", None, -1))
        self.btn_move_down.setText(QtWidgets.QApplication.translate("Script", "Descendre", None, -1))
        self.label_file.setToolTip(QtWidgets.QApplication.translate("Script", "Nom du fichier courant", None, -1))
        self.label_file.setText(QtWidgets.QApplication.translate("Script", "<aucun>", None, -1))
        self.btn_open.setToolTip(QtWidgets.QApplication.translate("Script", "Ouvrir un fichier de script", None, -1))
        self.btn_open.setText(QtWidgets.QApplication.translate("Script", "Ouvrir", None, -1))
        self.btn_save.setToolTip(QtWidgets.QApplication.translate("Script", "Enregistrer le script", None, -1))
        self.btn_save.setText(QtWidgets.QApplication.translate("Script", "Sauvegarder", None, -1))
        self.btn_export.setToolTip(QtWidgets.QApplication.translate("Script", "Exporter le script", None, -1))
        self.btn_export.setText(QtWidgets.QApplication.translate("Script", "Exporter", None, -1))
